...
MediaManagement::setUploadPathAndURLToPublic();
MediaManagement::addSize("featured", 565, 375)->addSize("medium", 540, 360);

add_filter(
    BASE_FILTER_BEFORE_RENDER_FORM,
    function ($form, $data) {
        switch (get_class($data)) {
            case Post::class:
            case Page::class:
                $bannerImage = MetaBox::getMetaData(
                    $data,
                    "banner_image",
                    true
                );

                $form->addAfter("image", "banner_image", "mediaImage", [
...
Form::component(
    "themeIcon",
    Theme::getThemeNamespace() . "::partials.icons-field",
