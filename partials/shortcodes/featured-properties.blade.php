@php
    $featured = get_featured_properties(5, [
        'author',
                        ...
                        <div class="post-group__left">
                            <article class="post post__inside post__inside--feature">
                                <div class="post__thumbnail">
                                    <img src="{{ MediaManagement::getImageUrl($featureItem->image, 'featured', false, MediaManagement::getDefaultImage()) }}" alt="{{ $featureItem->name }}"><a href="{{ $featureItem->url }}" class="post__overlay"></a>
                                </div>
                            ...
                            @else
                                <div class="post-group__item">
                                    <article class="post post__inside post__inside--feature post__inside--feature-small">
                                        <div class="post__thumbnail"><img src="{{ MediaManagement::getImageUrl($featureItem->image, 'medium', false, MediaManagement::getDefaultImage()) }}" alt="{{ $featureItem->name }}"><a href="{{ $featureItem->url }}" class="post__overlay"></a></div>
                                        <header class="post__header">
                                            <h3 class="post__title"><a href="{{ $featureItem->url }}">{{ $featureItem->name }}</a></h3>
                                        </header>
                                    </article>
